...
if (!$table_exists) {
    ...
    if (!in_array('item_hash', $column_names)) {
        ...
        if ($result === false) {
            echo '<p style="color: red;">Fehler beim Hinzufügen der item_hash-Spalte: ' . $wpdb->last_error . '</p>';
        } else {
    ...
    foreach ($required_columns as $column => $definition) {
        if (!in_array($column, $column_names)) {
            ...
            $wpdb->query("ALTER TABLE {$wpdb->prefix}feed_raw_items ADD COLUMN $column $definition");
...
echo '<p>Die Datenbankstruktur wurde erfolgreich aktualisiert. Sie können nun zum <a href="' . admin_url('admin.php?page=athena-feed-items') . '">Feed-Management</a> zurückkehren.</p>';
