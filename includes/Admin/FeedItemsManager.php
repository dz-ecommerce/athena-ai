...
class FeedItemsManager extends BaseAdmin {
    ...
    public function get_feed_item_content(): void {
        // Überprüfe die Nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'athena_get_feed_item_content')) {
            wp_send_json_error(['message' => __('Security check failed.', 'athena-ai')]);
        }
        
        // Überprüfe Berechtigungen
        if (!current_user_can('read')) {
            wp_send_json_error(['message' => __('You do not have permission to view this content.', 'athena-ai')]);
        }
        ...
        $item = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}feed_raw_items WHERE id = %d",
            $item_id
        ));
        ...
        if ($feed_title) {
            $content .= '<span class="athena-badge bg-blue-100 text-blue-800"><i class="fa-solid fa-rss mr-1"></i> ' . esc_html($feed_title) . '</span>';
        }
        
        // Publikationsdatum
        if ($item->pub_date) {
            $pub_date = strtotime($item->pub_date);
            $content .= '<span class="athena-badge bg-gray-100 text-gray-800"><i class="fa-solid fa-calendar mr-1"></i> ' . date_i18n(get_option('date_format'), $pub_date) . '</span>';
        }
        
        // Autoren
        if (!empty($author)) {
            $content .= '<span class="athena-badge bg-purple-100 text-purple-800"><i class="fa-solid fa-user mr-1"></i> ' . esc_html($author) . '</span>';
        }
        ...
        if (!empty($description)) {
            ...
            $content .= '<div class="italic text-gray-500">' . esc_html__('No content available.', 'athena-ai') . '</div>';
        }
    ...
    public function delete_feed_item() {
        // Überprüfe die Nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'athena_delete_feed_item')) {
            wp_send_json_error(['message' => __('Security check failed.', 'athena-ai')]);
        }
        
        // Überprüfe Berechtigungen
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('You do not have permission to delete feed items.', 'athena-ai')]);
        }
