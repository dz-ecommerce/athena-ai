...
class FeedFetcher {
    ...
    public static function init(): void {
        ...
        \add_action('admin_post_athena_debug_fetch_feeds', function () {
            if (!\current_user_can('manage_options')) {
                \wp_die(
                    \__('You do not have sufficient permissions to access this page.', 'athena-ai')
                );
            }
            ...
            $logger->info(
                sprintf(
                    'Fetch result - Success: %d, Errors: %d, New items: %d',
                    $result['success'],
                    $result['error'],
                    $result['new_items']
                )
            ...
            echo '<p>' .
                \__('Feed fetch completed. Redirecting in 3 seconds...', 'athena-ai') .
                '</p>';
            ...
            echo '<script>setTimeout(function(){ window.location.href = "' .
                \AthenaAI\Core\SafetyWrapper::esc_url($redirect_url) .
                '"; }, 3000);</script>';
            echo '</body></html>';
            exit();
        });
    ...
    public static function fetch_all_feeds(
        ...
        if ($verbose_console) {
            ...
            echo '<script>console.log("WordPress Version: ' .
        ...
        if ($verbose_console) {
            echo '<script>console.log("Athena AI Feed Fetcher: Found ' .
                count($feeds) .
                ' feeds to update");</script>';
        }
        ...
        if ($debug_mode) {
            $logger->info(
                sprintf(
                    'Feed fetch process completed. Success: %d, Errors: %d, New items: %d',
                    $results['success'],
                    $results['error'],
                    $total_new_items
                )
        ...
        if ($verbose_console) {
            echo '<script>console.log("Athena AI Feed Fetcher: Feed fetch process completed. Success: ' .
                $results['success'] .
                ', Errors: ' .
                $results['error'] .
                ', New items: ' .
            ...
            if (!empty($results['details'])) {
                ...
                foreach ($results['details'] as $index => $error_message) {
                    echo '<script>console.error("Error ' .
                        ($index + 1) .
                        ': ' .
                        \AthenaAI\Core\SafetyWrapper::esc_js($error_message) .
                        '");</script>';
                }
     ...
     */
    private static function process_single_feed(
        $feed_post,
        LoggerService $logger,
        bool $debug_mode,
        bool $verbose_console
    ): array {
        global $wpdb;
        ...
        try {
            ...
            if (!self::ensure_feed_metadata_exists($feed_post->ID)) {
                ...
                if ($verbose_console) {
                    echo '<script>console.error("Athena AI Feed Fetcher: Failed to ensure feed metadata for feed: ' .
                        \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                        '");</script>';
                }
                $result['error']++;
                $result['details'][] =
                    'Failed to ensure feed metadata for feed: ' . $feed_post->post_title;
                return $result;
            }
            ...
            if (empty($feed_url)) {
                ...
                if ($verbose_console) {
                    echo '<script>console.error("Athena AI Feed Fetcher: Feed URL is empty for feed: ' .
                        \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                        '");</script>';
                }
                $result['error']++;
            ...
            if ($verbose_console) {
                echo '<script>console.log("Athena AI Feed Fetcher: Processing feed: ' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                    ' (ID: ' .
                    $feed_post->ID .
                    ', URL: ' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($feed_url) .
                    ')");</script>';
            }
            ...
            $logger->debug(
                sprintf('Feed %s has %d items before fetch', $feed_post->post_title, $before_count)
            );

            // Get feed update interval
            $feed_update_interval = (int) \get_post_meta(
                $feed_post->ID,
                '_athena_feed_update_interval',
                true
            );
            ...
            $feed = new Feed(\esc_url_raw($feed_url), $feed_update_interval);
            ...
            if ($success) {
                ...
                $logger->debug(
                    sprintf(
                        'Feed %s has %d items after fetch',
                        $feed_post->post_title,
                        $after_count
                    )
                );
                ...
                $logger->info(
                    sprintf(
                        'Successfully fetched feed: %s. New items: %d',
                        $feed_post->post_title,
                        $new_items
                    )
                );
                ...
                if ($verbose_console) {
                    ...
                    if (!empty($last_error)) {
                        echo '<script>console.error("Athena AI Feed Fetcher: Failed to fetch feed: ' .
                            \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                            ' - Error: ' .
                            \AthenaAI\Core\SafetyWrapper::esc_js($last_error) .
                            '");</script>';
                    } else {
                        echo '<script>console.error("Athena AI Feed Fetcher: Failed to fetch feed: ' .
                            \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                            ' - Unknown error");</script>';
                    }
                }
            }
        } catch (\Exception $e) {
            $result['error']++;
            $result['details'][] =
                'Error processing feed ' . $feed_post->post_title . ': ' . $e->getMessage();

            $logger->error(sprintf('Exception while processing feed: %s', $e->getMessage()));

            if ($verbose_console) {
                echo '<script>console.error("Athena AI Feed Fetcher: Exception while processing feed ' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($feed_post->post_title) .
                    ': ' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($e->getMessage()) .
                    '");</script>';
            }
        }
    ...
    public static function check_and_update_schema(): void {
        ...
        try {
            ...
            $logger = null;
            if ($debug_mode) {
                $logger = \AthenaAI\Services\LoggerService::getInstance()->setComponent(
                    'FeedFetcher'
                );
            }
    ...
    private static function check_and_update_feed_metadata_table($logger = null): void {
        ...
        $table_exists =
            $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}feed_metadata'") ===
        ...
        if ($columns === false || $columns === null) {
            if ($logger) {
                $logger->error(
                    'Failed to get columns from feed_metadata table. Error: ' . $wpdb->last_error
                );
            }
            return;
        }
        ...
        if (!in_array('last_fetched', $column_names)) {
            $result = $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN last_fetched DATETIME DEFAULT NULL"
            );
            if ($logger) {
                $logger->info(
                    'Added missing column last_fetched to feed_metadata table. Result: ' .
        ...
        if (!in_array('fetch_interval', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN fetch_interval INT DEFAULT 3600"
            );
        }

        if (!in_array('fetch_count', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN fetch_count INT DEFAULT 0"
            );
        }

        if (!in_array('last_error_date', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN last_error_date DATETIME DEFAULT NULL"
            );
        }

        if (!in_array('last_error_message', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN last_error_message TEXT"
            );
        }

        if (!in_array('created_at', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP"
            );
        }

        if (!in_array('updated_at', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_metadata ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP"
            );
        }
    }
    ...
    private static function check_and_update_feed_raw_items_table($logger = null): void {
        ...
        if ($columns === false || $columns === null) {
            if ($logger) {
                $logger->error(
                    'Failed to get columns from feed_raw_items table. Error: ' . $wpdb->last_error
                );
            }
            return;
        }
        ...
        if (!in_array('item_hash', $column_names)) {
            ...
            if ($result === false) {
                if ($logger) {
                    $logger->error(
                        'Failed to add item_hash column to feed_raw_items table. Error: ' .
                            $wpdb->last_error
                    );
                }
                return;
        ...
        if (!in_array('raw_content', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_raw_items ADD COLUMN raw_content LONGTEXT"
            );
        ...
        if (!in_array('created_at', $column_names)) {
            $wpdb->query(
                "ALTER TABLE {$wpdb->prefix}feed_raw_items ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP"
            );
    ...
    private static function ensure_feed_metadata_exists(int $feed_id): bool {
        ...
        if (!$table_exists) {
            if ($debug_mode) {
                \error_log(
                    'Athena AI: Feed metadata table does not exist. Cannot ensure metadata.'
                );
            }
            if ($verbose_console) {
        ...
        try {
            ...
            if (!$exists && !empty($feed_url)) {
                ...
                if ($exists_by_url) {
                    if ($debug_mode) {
                        \error_log(
                            "Athena AI: Feed metadata already exists with URL {$feed_url} for feed ID {$exists_by_url}, but trying to create for feed ID {$feed_id}"
                        );
                    }
                    ...
                    if ($exists_by_url != $feed_id) {
                        ...
                        if ($debug_mode) {
                            \error_log(
                                "Athena AI: Updated feed metadata from feed ID {$exists_by_url} to {$feed_id}. Result: " .
            ...
            if (!$exists) {
                ...
                if (!$columns) {
                    if ($debug_mode) {
                        \error_log(
                            'Athena AI: Failed to get columns from feed_metadata table. Error: ' .
                                $wpdb->last_error
                        );
                    }
                    return false;
                ...
                if ($debug_mode) {
                    \error_log(
                        'Athena AI: Feed metadata table columns: ' . implode(', ', $column_names)
                    );
                    \error_log(
                        "Athena AI: Feed ID: {$feed_id}, Update interval: {$update_interval}"
                    );
                }
                ...
                $result = $wpdb->insert($wpdb->prefix . 'feed_metadata', $data, $format_types);

                if ($result === false) {
                    if ($debug_mode) {
                        \error_log(
                            "Athena AI: Failed to insert feed metadata for feed ID {$feed_id}. Error: " .
                                $wpdb->last_error
                        );
                    }
                    if ($verbose_console) {
                        echo '<script>console.error("Athena AI Feed Fetcher: Failed to insert feed metadata for feed ID ' .
                            $feed_id .
                            '. Database error: ' .
                            \AthenaAI\Core\SafetyWrapper::esc_js($wpdb->last_error) .
                            '");</script>';
                        echo '<script>console.log("Data being inserted: ", ' .
                            \json_encode($data) .
                            ');</script>';
                        echo '<script>console.log("Format types: ", ' .
                            \json_encode($format_types) .
                            ');</script>';
                    }
                    return false;
                }
                ...
                if ($debug_mode) {
                    \error_log(
                        "Athena AI: Successfully created feed metadata for feed ID {$feed_id}"
                    );
                }
            ...
            if ($debug_mode) {
                \error_log(
                    'Athena AI: Exception in ensure_feed_metadata_exists: ' . $e->getMessage()
                );
            }
            if ($verbose_console) {
                echo '<script>console.error("Athena AI Feed Fetcher: Exception in ensure_feed_metadata_exists for feed ID ' .
                    $feed_id .
                    ': ' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($e->getMessage()) .
                    '");</script>';
                echo '<script>console.log("Exception trace: ", "' .
                    \AthenaAI\Core\SafetyWrapper::esc_js($e->getTraceAsString()) .
                    '");</script>';
            }
            return false;
        }
        ...
        if ($debug_mode) {
            ...
            if (!empty($all_feeds)) {
                ...
                foreach ($all_feeds as $feed) {
                    ...
                    $is_due =
                        $feed->last_fetched === null ||
                        $feed->seconds_since_last_fetch > $feed->fetch_interval;
