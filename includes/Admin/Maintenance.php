...
class Maintenance {
    ...
    public static function init(): void {
        ...
        add_action('wp_ajax_athena_ai_drop_feed_items_table', [self::class, 'ajax_drop_feed_items_table']);
        add_action('wp_ajax_athena_ai_create_feed_items_table', [self::class, 'ajax_create_feed_items_table']);
    }
    ...
    public static function render_maintenance_page(): void {
        ...
        echo '<thead><tr><th>' . esc_html__('Table', 'athena-ai') . '</th><th>' . esc_html__('Status', 'athena-ai') . '</th><th>' . esc_html__('Records', 'athena-ai') . '</th></tr></thead>';
        echo '<tbody>';
        
        // Feed Items table
        echo '<tr>';
        echo '<td><code>' . esc_html($feed_items_table) . '</code></td>';
        echo '<td>' . ($feed_items_exists ? '<span style="color: green;">' . esc_html__('Exists', 'athena-ai') . '</span>' : '<span style="color: red;">' . esc_html__('Missing', 'athena-ai') . '</span>') . '</td>';
        ...
        echo '<td>' . ($feed_metadata_exists ? '<span style="color: green;">' . esc_html__('Exists', 'athena-ai') . '</span>' : '<span style="color: red;">' . esc_html__('Missing', 'athena-ai') . '</span>') . '</td>';
        ...
        echo '<p>' . esc_html__('These operations allow you to manage feed items data for testing and troubleshooting.', 'athena-ai') . '</p>';
        ...
        echo '<p><em>' . esc_html__('Debug information will appear here...', 'athena-ai') . '</em></p>';
        echo '</div>';
        ...
        jQuery(document).ready(function($) {
            ...
            $('#truncate-feed-items').on('click', function() {
                if (!confirm('<?php echo esc_js(__('Are you sure you want to empty the feed items table? This cannot be undone.', 'athena-ai')); ?>')) {
                    return;
                }
                
                addDebugMessage('<?php echo esc_js(__('Emptying feed items table...', 'athena-ai')); ?>');
                ...
                $.ajax({
                    ...
                    error: function() {
                        addDebugMessage('<?php echo esc_js(__('AJAX error occurred.', 'athena-ai')); ?>', true);
            ...
            $('#drop-feed-items-table').on('click', function() {
                if (!confirm('<?php echo esc_js(__('Are you sure you want to completely remove the feed items table? This cannot be undone.', 'athena-ai')); ?>')) {
                    return;
                }
                
                addDebugMessage('<?php echo esc_js(__('Dropping feed items table...', 'athena-ai')); ?>');
                $(this).prop('disabled', true);
            ...
            $('#create-feed-items-table').on('click', function() {
                addDebugMessage('<?php echo esc_js(__('Creating feed items table...', 'athena-ai')); ?>');
                $(this).prop('disabled', true);
    ...
    public static function ajax_truncate_feed_items(): void {
        ...
        if ($result !== false) {
            wp_send_json_success(['message' => __('Feed items table has been emptied successfully.', 'athena-ai')]);
        } else {
            wp_send_json_error(['message' => __('Failed to empty feed items table: ', 'athena-ai') . $wpdb->last_error]);
        }
    }
    ...
    public static function ajax_drop_feed_items_table(): void {
        ...
        if ($result !== false) {
            wp_send_json_success(['message' => __('Feed items table has been dropped successfully.', 'athena-ai')]);
        } else {
            wp_send_json_error(['message' => __('Failed to drop feed items table: ', 'athena-ai') . $wpdb->last_error]);
        }
    }
    ...
    public static function ajax_create_feed_items_table(): void {
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") === $table_name) {
            wp_send_json_success(['message' => __('Feed items table has been created successfully.', 'athena-ai')]);
        } else {
            wp_send_json_error(['message' => __('Failed to create feed items table. Error: ', 'athena-ai') . $wpdb->last_error]);
        }
    }
}
