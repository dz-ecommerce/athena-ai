...
class FeedItemsPage {
    ...
    public static function init(): void {
        ...
        add_action('admin_post_athena_debug_cron_health', [
            self::class,
            'handle_debug_cron_health',
        ]);
    }
    ...
    public static function render_page(): void {
        ...
        if (!SchemaManager::tables_exist()) {
            ...
            if (!SchemaManager::tables_exist()) {
                wp_die(
                    __(
                        'Database tables could not be created. Please check your database permissions.',
                        'athena-ai'
                    )
                );
            }
        ...
        $feed_filter =
            isset($_GET['feed_id']) && !empty($_GET['feed_id']) ? intval($_GET['feed_id']) : 0;
        $date_filter =
            isset($_GET['date_filter']) && !empty($_GET['date_filter'])
        ...
        $feed_filter_ids =
            isset($_GET['feed_ids']) && is_array($_GET['feed_ids'])
    ...
    private static function clean_up_feeds(): void {
        ...
        $wpdb->query(
            "DELETE FROM {$wpdb->prefix}feed_raw_items WHERE feed_id NOT IN ($feed_ids_string)"
        );
    }
    ...
    public static function handle_fetch_feeds(): void {
        ...
        if (
            ...
            !wp_verify_nonce($_POST['_wpnonce'], 'athena_fetch_feeds_nonce')
        ) {
            wp_die(__('Security check failed. Please try again.', 'athena-ai'));
        }
    ...
    public static function handle_manual_fetch(): void {
        ...
        if ($result) {
            ...
            wp_send_json_error([
                'message' => __(
                    'Failed to fetch feed. Please check the feed URL and try again.',
                    'athena-ai'
                ),
            ]);
        }
