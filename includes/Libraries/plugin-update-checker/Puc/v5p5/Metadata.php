    ...
    abstract class Metadata {
        ...
        public static function fromJson($json) {
            throw new LogicException(
                'The ' . __METHOD__ . ' method must be implemented by subclasses'
            );
        }
        ...
        protected static function createFromJson($json, $target) {
            ...
            if (empty($apiResponse) || !is_object($apiResponse)) {
                $errorMessage =
                    'Failed to parse update metadata. Try validating your .json file with https://jsonlint.com/';
        ...
        protected function copyFields($from, $to) {
            ...
            if (property_exists($from, 'slug') && !empty($from->slug)) {
                ...
                $fields = apply_filters(
                    $this->getPrefixedFilter('retain_fields') . '-' . $from->slug,
                    $fields
                );
            }
