...
if (!class_exists(Extension::class, false)):
    class Extension {
        ...
        public function __construct($updateChecker, $panelClass = null) {
            ...
            add_action('wp_ajax_puc_v5_debug_check_now', [$this, 'ajaxCheckNow']);
        }
        ...
        public function ajaxCheckNow() {
            ...
            if (
                !isset($_POST['uid']) ||
                $_POST['uid'] !== $this->updateChecker->getUniqueName('uid')
            ) {
                return;
            }
            $this->preAjaxRequest();
            ...
            if (!empty($errors)) {
                ...
                foreach (array_values($errors) as $num => $item) {
                    ...
                    echo '<dl>';
                    printf(
                        '<dt>Error code:</dt><dd><code>%s</code></dd>',
                        esc_html($wpError->get_error_code())
                    );

                    if (isset($item['url'])) {
                        printf(
                            '<dt>Requested URL:</dt><dd><code>%s</code></dd>',
                            esc_html($item['url'])
                        );
                    }
                    ...
                    if (isset($item['httpResponse'])) {
                        if (is_wp_error($item['httpResponse'])) {
                            ...
                            foreach (
                                wp_remote_retrieve_headers($item['httpResponse'])
                                as $name => $value
                            ) {
                            ...
                            if ($body === '') {
                                ...
                                $body =
                                    ...
                                    sprintf(
                                        "\n(Long string truncated. Total length: %d bytes.)",
                                        $length
                                    );
                            }

                            printf(
                                '<dt>Response body:</dt><dd><pre>%s</pre></dd>',
                                esc_html($body)
                            );
                        }
                    }
        ...
        public function removeHooks() {
            ...
            remove_action('wp_ajax_puc_v5_debug_check_now', [$this, 'ajaxCheckNow']);
        }
        ...
        private function getLibraryUrl($filePath) {
            ...
            if (
                strpos($absolutePath, $pluginDir) === 0 ||
                strpos($absolutePath, $muPluginDir) === 0
            ) {
