    ...
    class GitLabApi extends Api {
            ...
            if (
                ...
                if (count($parts) < 3) {
                    throw new \InvalidArgumentException(
                        'Invalid GitLab.com repository URL: "' . $repositoryUrl . '"'
                    );
                }
                ...
                if (count($segments) < 2) {
                    throw new \InvalidArgumentException(
                        'Invalid GitLab repository URL: "' . $repositoryUrl . '"'
                    );
                }
                ...
                if (count($segments) > 0) {
                    $this->repositoryHost =
                        trailingslashit($this->repositoryHost) . implode('/', $segments);
                }
        ...
        public function getLatestRelease() {
            ...
            foreach ($releases as $release) {
                if (
                    ...
                    (!empty($release->upcoming_release) && $this->shouldSkipPreReleases())
                ) {
                    continue;
                }
        ...
        public function getTag($tagName) {
            throw new \LogicException(
                'The ' . __METHOD__ . ' method is not implemented and should not be used.'
            );
        }
        ...
        protected function getUpdateDetectionStrategies($configBranch) {
            ...
            if ($configBranch === 'main' || $configBranch === 'master') {
                $strategies[self::STRATEGY_LATEST_RELEASE] = [$this, 'getLatestRelease'];
