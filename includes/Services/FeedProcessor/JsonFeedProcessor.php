...
class JsonFeedProcessor extends AbstractFeedProcessor {
    ...
    protected function processItem(array $item): ?array {
        ...
        $processed_item = [
            ...
            'description' => $this->cleanFieldValue(
                $item['description'] ?? ($item['summary'] ?? ($item['excerpt'] ?? null))
            ),
            'content' => $this->cleanFieldValue(
                $item['content'] ??
                    ($item['body'] ?? ($item['text'] ?? ($item['description'] ?? null)))
            ),
            'pub_date' => $pub_date,
    ...
    protected function extractEnclosures(array $item): array {
        ...
        if (isset($item['enclosures']) && is_array($item['enclosures'])) {
            foreach ($item['enclosures'] as $enclosure) {
                if (
                    (is_array($enclosure) && isset($enclosure['url'])) ||
                    isset($enclosure['link'])
                ) {
                    $enclosures[] = [
