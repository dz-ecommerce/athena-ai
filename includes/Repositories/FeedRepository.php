...
class FeedRepository {
    ...
    public function save(Feed $feed) {
        ...
        if (\is_wp_error($post_id)) {
            $this->logger->error(
                sprintf('Failed to create feed post: %s', $post_id->get_error_message())
            );
            return false;
        }
    ...
    public function ensure_feed_metadata_exists(Feed $feed): bool {
        ...
        $exists = $wpdb->get_var(
            $wpdb->prepare("SELECT COUNT(*) FROM {$metadata_table} WHERE feed_id = %d", $feed_id)
        );
    ...
    public function update_feed_error(Feed $feed, string $code, string $message): bool {
        ...
        $error_message = sprintf('[%s] %s - %s', $code, \current_time('mysql'), $message);
    ...
    public function log_error(Feed $feed, string $code, string $message): void {
        ...
        $feed_id_info =
            $feed->get_post_id() !== null
                ? "(Feed ID: {$feed->get_post_id()})"
                : "(URL: {$feed->get_url()})";
        \error_log(sprintf('Athena AI Feed Error [%s]: %s %s', $code, $message, $feed_id_info));
    }
    ...
    public function process_feed_items(Feed $feed, array $items): array {
        ...
        try {
            foreach ($items as $item) {
                ...
                $pub_date = function_exists('current_time')
                    ? \current_time('mysql')
                    : date('Y-m-d H:i:s');
